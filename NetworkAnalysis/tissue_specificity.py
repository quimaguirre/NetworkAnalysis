...
class HouseKeepingGenes(object):
    ...
    def __init__(self, type_id, pickles_path):
        """ 
        @param:    type_id
...
def filter_network_tissue_specific(input_network, tissue, permission, output_network, output_nodes=None):
    """
    Gets a complete network and filters by tissue interactions in the same tissue.

    @param:    input_network
    @pdef:     Network that has to be filtered
    @ptype:    Network file

    @param:    tissue
    @pdef:     Tissue of interest
    @ptype:    NetworkAnalysis Tissue object

    @param:    permission
    @pdef:     Level of permission of the filtering (0 = high, 1 = medium, 2 = low)
    @ptype:    Integer

    @param:    output_network
    @pdef:     Filtered network
    @ptype:    Network file
