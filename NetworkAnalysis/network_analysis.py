...
class Network(object):
    ...
    def __init__(self, network_file, type_id, network_format, node_file=None):
        """ 
        @param:    network_file
        ...
        @param:    node_file
        @pdef:     Path to the file containing the nodes of the network. If not available, introduce None
        ...
        self.pickles_path = os.path.join(os.path.dirname(__file__), 'pickles')
    ...
    def get_methodid2interactions(self, verbose=False):
        ...
        for u,v,method_ids in self.network.edges.data("method_ids"):
    ...
    def get_method2interactions(self, verbose=False):
        ...
        for u,v,method_names in self.network.edges.data("method_names"):
    ...
    def get_nummethodids2interactions(self, verbose=False):
        """
        Obtains a dictionary containing the number of interactions containing a 
        given number of method IDs
        If verbose:
        Prints the number of methods and number of interactions in order of
        number of methods (higher to lower)
        """
        if self.network_format != 'multi-fields':
            print('It is only possible to use this method with a multi-fields network\n')
            sys.exit(10)

        nummethodids2interactions = {}

        for u,v,method_ids in self.network.edges.data("method_ids"):

            num_methodids = len(method_ids)
            nummethodids2interactions.setdefault(num_methodids, 0)
            nummethodids2interactions[num_methodids] += 1

        if verbose:
            for num_methodids, interactions in sorted(nummethodids2interactions.iteritems(), key=lambda (x, y): x):
                print('Number of methods: {}\tNumber of interactions: {}'.format(num_methodids, interactions))

        return nummethodids2interactions
    ...
    def get_numpmids2interactions(self, verbose=False):
        ...
        for u,v,pmids in self.network.edges.data("pmids"):
    ...
    def get_database2interactions(self, verbose=False):
        ...
        for u,v,sources in self.network.edges.data("sources"):
    ...
    def translate_network(self, translation_file, translation_id, translation_format, translated_network, translated_nodes=None):
        """
        ...
        NT.translate(self.network_file, translated_network, translation_file, self.network_format, translation_format, self.node_file, translated_nodes)

        return Network(translated_network, translation_id, translation_format, translated_nodes)
    ...
    def filter_network_by_tissue(self, filtered_network_file, tissue_object, permission, filtered_nodes_file=None, verbose=False):
        """
        ...
        return TissueSpecificNetwork(filtered_network_file, self.type_id, self.network_format, tissue_object, permission, filtered_nodes_file)
    ...
    def filter_network_by_method(self, methods_excluded=None, method_ids_excluded=None, methods_included=None, method_ids_included=None, output_network_file=None, output_nodes_file=None, verbose=False):
        ...
        for u,v,d in self.network.edges(data=True):
            ...
            if methods_included != None:
                if len(methods_included)>0:
                    skip_inc = True
                    for method in d['method_names']:
                        if method in methods_included:
                            skip_inc = False
            if method_ids_included != None:
                if len(method_ids_included)>0:
                    method_ids_included = [str(x) for x in method_ids_included]
                    skip_inc = True
                    for method in d['method_ids']:
                        if method in method_ids_included:
                            skip_inc = False
        ...
        write_network_file_from_networkx_graph(fnet, output_network_file, self.network_format, output_nodes_file, tissue_specific=self._tissue_specific)

        if verbose:
            print('Filtering network by method... finished!\n')

        return Network(output_network_file, self.type_id, self.network_format, output_nodes_file)

    def filter_network_by_number_pubmeds(self, min_num_pubmeds, output_network_file, output_nodes_file=None, verbose=False):
        """
    ...
    def filter_network_by_number_databases(self, min_num_databases, output_network_file, output_nodes_file=None, verbose=False):
        """
        Filter the network by minimum number of databases ids

        @param:    min_num_databases
        @pdef:     Minimum number of databases which an interaction has to have
        @ptype:    Integer
        ...
        if verbose:
            print('Filtering network by number of databases...\n')
        ...
        for u,v,d in self.network.edges(data=True):
            ...
            # Check if it has at least the minimum number of databases required
            number_databases = len(d['sources'])
            if number_databases >= min_num_databases:
                fnet.add_edge(u,v,d)

        write_network_file_from_networkx_graph(fnet, output_network_file, self.network_format, output_nodes_file, tissue_specific=self._tissue_specific)

        if verbose:
            print('Filtering network by number of databases... finished!\n')

        return Network(output_network_file, self.type_id, self.network_format, output_nodes_file)

    def filter_network_by_number_methods(self, min_num_methods, output_network_file, output_nodes_file=None, verbose=False):
        """
        Filter the network by minimum number of method ids

        @param:    min_num_methods
        @pdef:     Minimum number of methods which an interaction has to have
        @ptype:    Integer
        ...
        if verbose:
            print('Filtering network by number of methods...\n')
        ...
        for u,v,d in self.network.edges(data=True):
            ...
            # Check if it has at least the minimum number of method ids required
            number_methods = len(d['method_ids'])
            if number_methods >= min_num_methods:
                fnet.add_edge(u,v,d)

        write_network_file_from_networkx_graph(fnet, output_network_file, self.network_format, output_nodes_file, tissue_specific=self._tissue_specific)

        if verbose:
            print('Filtering network by number of methods... finished!\n')

        return Network(output_network_file, self.type_id, self.network_format, output_nodes_file)

    def filter_network_by_database(self, databases_included, output_network_file, output_nodes_file=None, verbose=False):
        """
    ...
    def remove_database_from_network(self, database_to_remove, output_network_file, output_nodes_file=None, verbose=False):
        """
        Filter the network by interactions included only in certain databases.
        If the interaction is from one of the databases, it is included

        @param:    database_to_remove
        @pdef:     Name of the database to remove
        @ptype:    List
        ...
        for u,v,d in self.network.edges(data=True):
            ...
            if database_to_remove in d['sources']:
                if len(d['sources']) > 1:
                    d['sources'].remove(database_to_remove)
                    fnet.add_edge(u,v,d)
            else:
                fnet.add_edge(u,v,d)
...
class TissueSpecificNetwork(Network):
    ...
    def __init__(self, network_file, type_id, network_format, tissue_object, permission, node_file=None):
        """ 
        @param:    network_file
        @pdef:     Path to the file containing the edges of the network
        @ptype:    {String}
    ...
    def translate_network(self, translation_file, translation_id, translation_format, translated_network, translated_nodes=None, verbose=False):
        """
        Uses a BIANA translation file to return a new Network object with the 
        desired type of IDs and format
        """
        ...
        return TissueSpecificNetwork(translated_network, translation_id, translation_format, self.tissue_object, self.permission, translated_nodes)
...
class GUILDProfile(object):
    ...
    def translate_pvalue_file(self, translation_file, translation_id, output_file, verbose=False):
        ...
        NT.translate(None, None, translation_file, 'guild', 'guild', self.pvalue_file, output_file)
...
class EdgeProfile(Network):
    ...
    def translate_network(self, translation_file, translation_id, output_file, verbose=False):
        ...
        NT.translate(self.network_file, output_file, translation_file, self.network_format, self.network_format, None, None)
...
class Tissue(object):
    ...
    def __init__(self, tissue_terms_hpa, tissue_terms_jensen, jensen_conf=3, hpa_tpm_threshold=1, hpa_level='medium', hpa_rel='approved', pickles_path=os.path.join(os.path.dirname(__file__), 'pickles'), verbose=False):
        """ 
        ...
        self.hpa_tpm_threshold = hpa_tpm_threshold
        ...
        prot2HPAmic_file = os.path.join(self.pickles_path,'UEprot2UEHPAmic.pcl')
        prot2HPARNAseq_file = os.path.join(self.pickles_path,'UEprot2UEHPARNAseq.pcl')
        self.UEprot2UETissues = cPickle.load(open(prot2tissues_file))
        self.UEprot2UEHPAmic = cPickle.load(open(prot2HPAmic_file))
        self.UEprot2UEHPARNAseq = cPickle.load(open(prot2HPARNAseq_file))

        # Get all the tissue-specific proteins
        self.proteins_hpa_mic = self.get_tissue_specific_proteins_hpa_microarray()
        self.proteins_hpa_rna = self.get_tissue_specific_proteins_hpa_rnaseq()
        self.proteins_jensen = self.get_tissue_specific_proteins_jensen()
        self.all_tissue_proteins = self.proteins_hpa_mic | self.proteins_hpa_rna | self.proteins_jensen
    ...
    def get_user_entities_jensen(self): 
        ...
        for term in self.tissue_terms_jensen:
            if term in self.BTOname2uE:
                for uE in self.BTOname2uE[term]:
                    self.user_entities_jensen.add(uE)
    ...
    def get_tissue_specific_proteins_hpa_microarray(self): 
        """ 
        ...
        """
        proteins_hpa_mic = set()
        prot_hpa_mic_to_values = {}
        for uE_protein in self.UEprot2UEHPAmic:
            for uE_tissue in self.UEprot2UEHPAmic[uE_protein]:
                if uE_tissue in self.user_entities_hpa:
                    level = self.UEprot2UEHPAmic[uE_protein][uE_tissue]['level']
                    reliability = self.UEprot2UEHPAmic[uE_protein][uE_tissue]['reliability']
                    ...
                    if self.hpa_scale_rel[reliability] >= self.hpa_scale_rel[self.hpa_rel]:
                        ...
                        if self.hpa_scale_level[level] >= self.hpa_scale_level[self.hpa_level]:
                            proteins_hpa_mic.add(uE_protein)
                            prot_hpa_mic_to_values.setdefault(uE_protein, {})
                            prot_hpa_mic_to_values[uE_protein].setdefault(uE_tissue, {})
                            prot_hpa_mic_to_values[uE_protein][uE_tissue]['level'] = level
                            prot_hpa_mic_to_values[uE_protein][uE_tissue]['reliability'] = reliability

        return proteins_hpa_mic

    def get_tissue_specific_proteins_hpa_rnaseq(self): 
        """ 
        Returns all the user entities from Human Protein Atlas expressed in the
        tissues of interest above a certain level value of TPM units
        """
        proteins_hpa_rna = set()
        prot_hpa_rna_to_values = {}
        for uE_protein in self.UEprot2UEHPARNAseq:
            for uE_tissue in self.UEprot2UEHPARNAseq[uE_protein]:
                if uE_tissue in self.user_entities_hpa:
                    value = self.UEprot2UEHPARNAseq[uE_protein][uE_tissue]['value']
                    unit = self.UEprot2UEHPARNAseq[uE_protein][uE_tissue]['unit']
                    if unit != 'tpm':
                        print('Incorrect RNAseq unit for uE protein {}: {}'.format(uE_protein, unit))
                        sys.exit(10)

                    # If our TPM value is bigger or equal than the cut-off,
                    # we continue
                    if value >= self.hpa_tpm_threshold:
                        proteins_hpa_rna.add(uE_protein)
                        prot_hpa_rna_to_values.setdefault(uE_protein, {})
                        prot_hpa_rna_to_values[uE_protein].setdefault(uE_tissue, {})
                        prot_hpa_rna_to_values[uE_protein][uE_tissue]['value'] = value
                        prot_hpa_rna_to_values[uE_protein][uE_tissue]['unit'] = unit

        return proteins_hpa_rna
...
def write_network_file_from_networkx_graph(input_network, output_network_file, output_network_format, output_nodes_file=None, tissue_specific=False):
    """
    Writes a network file and a nodes file from a networkx graph object.
    (Currently only available for multi-fields networks)
    """

    with open(output_network_file, 'w') as output_network_fd:

        if output_network_format == 'multi-fields':
            for u,v,d in input_network.edges(data=True):
            ...
            for u,v in input_network.edges:
                output_network_fd.write('{}\t{}\n'.format( u,v ))
        elif output_network_format == 'sif':
            try:
                for u,v,d in input_network.edges(data=True):
                    if 'score' in d:
                        score = d['score']
                ...
                score = 1.000000
                for u,v in input_network.edges(data=True):
                    output_network_fd.write('{}\t{}\t{}\n'.format( u,score,v ))
        else:
            print('Not available format: {}.\n'.format(output_network_format))
            sys.exit(10)

    
    if output_nodes_file:
        with open(output_nodes_file, 'w') as output_nodes_fd:
            for node in input_network.nodes():
                output_nodes_fd.write('{}\n'.format(node))

    return

def write_node_and_edge_files_for_guildify_from_networkx_graph(input_network, type_id, output_node_file, output_edge_file, output_edge_file_diamond):
    """
    Writes a network file and a nodes file from a networkx graph object.
    (Currently only available for multi-fields networks)
    """
    ...
    up_table = return_unification_protocol_table(cnx, config.get('BIANA', 'unification_protocol'))
    ...
    for node in input_network.nodes():
        ...
        query = (''' SELECT UA.value, GS.value, D.value, GI.value 
                    FROM {} U1, externalEntityUniprotAccession UA, externalEntityGeneSymbol GS, externalEntityDescription D, externalEntityGeneID GI 
                    WHERE U1.externalEntityID = UA.externalEntityID AND U1.externalEntityID = GS.externalEntityID AND U1.externalEntityID = D.externalEntityID AND U1.externalEntityID = GI.externalEntityID 
                    AND UA.type = "unique" AND GS.type = "unique" AND GI.type = "unique" AND U1.userEntityID = %s
                '''.format(up_table))
        ...
        if len(rows) > 0:
           for items in rows:
               ...
               uniprot_acc, gene_symbol, description, geneid = items

               ue_to_uniprot.setdefault(node, set()).add(uniprot_acc)
               ue_to_gene_symbol.setdefault(node, set()).add(gene_symbol)
               ue_to_description.setdefault(node, set()).add(description)
               ue_to_geneid.setdefault(node, set()).add(str(geneid))
        else:
            discarded_nodes.add(node)

        """
        # Here I tried to do it separately to check if I could discard less nodes
        # However, more or less the nodes discarded are the same (30 nodes more)
        # What are the discarded nodes?
        # They are mainly intact proteins that have unreviewed uniprot accession codes
        # and therefore they are not in Uniprot database and do not have a gene ID associated.
        query1 = (''' SELECT UA.value
                      FROM {} U1, externalEntityUniprotAccession UA
                      WHERE U1.externalEntityID = UA.externalEntityID 
                      AND UA.type = "unique" AND U1.userEntityID = %s
                  '''.format(up_table))
        cursor.execute(query1, (node,))
        rows1 = cursor.fetchall()
        if len(rows1) > 0:
            for items in rows1:
                for uniprot_acc in items:
                    ue_to_uniprot.setdefault(node, set()).add(uniprot_acc)

        query2 = (''' SELECT GI.value 
                      FROM {} U1, externalEntityGeneID GI 
                      WHERE U1.externalEntityID = GI.externalEntityID 
                      AND GI.type = "unique" AND U1.userEntityID = %s
                  '''.format(up_table))
        cursor.execute(query2, (node,))
        ...
        query3 = (''' SELECT GS.value 
                      FROM {} U1, externalEntityGeneSymbol GS 
                      WHERE U1.externalEntityID = GS.externalEntityID 
                      AND GS.type = "unique" AND U1.userEntityID = %s
                  '''.format(up_table))
        cursor.execute(query3, (node,))
        rows3 = cursor.fetchall()
        if len(rows3) > 0:
            for items in rows3:
                for gene_symbol in items:
                    ue_to_gene_symbol.setdefault(node, set()).add(str(gene_symbol))

        query4 = (''' SELECT D.value 
                      FROM {} U1, externalEntityUniprotAccession UA, externalEntityDescription D 
                      WHERE U1.externalEntityID = UA.externalEntityID AND U1.externalEntityID = D.externalEntityID 
                      AND U1.userEntityID = %s
                  '''.format(up_table))
        cursor.execute(query4, (node,))
        rows4 = cursor.fetchall()
        if len(rows4) > 0:
            for items in rows4:
                for description in items:
                    ue_to_description.setdefault(node, set()).add(description.encode('utf-8'))

        if len(rows1) > 0 and len(rows2) > 0 and len(rows3) > 0:
            selected_nodes.add(node)
        else:
            discarded_nodes.add(node)
    ...
    for u,v,d in input_network.edges(data=True):
        if u in selected_nodes and v in selected_nodes:
            ...
            new_network[u][v].update(d)

    #print(discarded_nodes)
    print('Discarded nodes: {}'.format(len(discarded_nodes)))
    print('Initial nodes: {}. Selected nodes: {}'.format(len(input_network.nodes()), len(new_network.nodes())))
    print('Initial edges: {}. Selected edges: {}'.format(len(input_network.edges()), len(new_network.edges())))


    # Write Guildify node file
    with open(output_node_file, 'w') as output_nodes_fd:

        output_nodes_fd.write('BIANA ID\tUniProt ID\tgenesymbol\tdescription\tgeneid\tEquivalent Entries\n')

        for node in ue_to_uniprot:

            uniprot_accs = '-'
            gene_symbols = '-'
            descriptions = '-'
            geneids = '-'
            if node in ue_to_uniprot:
                uniprot_accs = '; '.join(ue_to_uniprot[node])
            if node in ue_to_gene_symbol:
                gene_symbols = '; '.join(ue_to_gene_symbol[node])
            if node in ue_to_description:
                descriptions = '; '.join(ue_to_description[node])
            if node in ue_to_geneid:
                geneids = '; '.join(ue_to_geneid[node])

            # BIANA ID  UniProt ID  genesymbol  description geneid  Equivalent Entries
            output_nodes_fd.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format(node, uniprot_accs, gene_symbols, descriptions, geneids, '-'))


    # Write Guildify edge file
    write_network_file_from_networkx_graph(new_network, output_edge_file, 'sif', output_nodes_file=None, tissue_specific=False)

    # Write Guildify edge file for DIAMOnD (in raw format)
    write_network_file_from_networkx_graph(new_network, output_edge_file_diamond, 'raw', output_nodes_file=None, tissue_specific=False)
...
#     for u,v in network1.network.edges:
#         comb1 = (u,v)
#         comb2 = (v,u)
...
def get_union_network_from_two_networks(network1, network2, output_network_file):
    """
    Gets network instance of the union of two networks (common and uncommon 
    elements)
    """
    if network1.network_format != network2.network_format:
        print('The format of network 1 ({}) is not the same as the format of network 2 ({}).\nPlease, introduce networks with the same format!'.format(network1.network_format, network2.network_format))
        sys.exit(10)
    if network1.type_id != network2.type_id:
        print('The type id of network 1 ({}) is not the same as the type id of network 2 ({}).\nPlease, introduce networks with the same type id!'.format(network1.type_id, network2.type_id))
        sys.exit(10)

    U = nx.compose(network1.network, network2.network)
    write_network_file_from_networkx_graph(U, output_network_file, network1.network_format, output_nodes_file=None, tissue_specific=False)
    union_network_instance = Network(output_network_file, network1.type_id, network1.network_format)

    return union_network_instance
...
def parse_ConsensusPathDB(ConsensusPathDB_file, output_network_file, output_network_format):
    """
    Parse the PPI network from ConsensusPathDB and return
    a network in raw/sif/multi-fields format.
    """
    G=nx.Graph()
    with open(ConsensusPathDB_file, 'r') as CPDB_fd:
        for line in CPDB_fd:
            if line[0] == '#':
                continue
    ...
    # Write the network
    write_network_file_from_networkx_graph(G, output_network_file, output_network_format, output_nodes_file=None, tissue_specific=False)

    return Network(output_network_file, 'uniprotentry', output_network_format)
